...
ladder on a fixed evaluation set, and 58 recorded audit-protocol runs, no configuration
makes the strong student exceed its own prefill: gold-probability change
spans $-0.14$ to $-0.30$ against a self-kv control at $+0.000$. A held-out
...
$-0.30$ across the mapper ladder (Section~\ref{sec:results}), while the weak
...
Joint MLP, c=30, 1.7B & 0.233 / 0.238 / 31.6 & $-0.265$ [$-0.426, -0.111$] & 30 \\
...
that reads all teacher layers and all KV heads also fails: CHG $-0.265$
$[-0.426, -0.111]$ with accuracy $0.233$, even though it reconstructs a fluent
cache (PPL $31.6$ versus $25.4$ for the student), reinforcing the
perplexity--accuracy decoupling. The strong student never
...
head dimension. No row recovers the teacher's gold probability ($0.691$).}
\label{tab:crossarch}
\footnotesize
\begin{tabular}{lcccc}
\toprule
...
zero-re-prefill regime. The mapper-ladder results use a single seed per configuration; because the
