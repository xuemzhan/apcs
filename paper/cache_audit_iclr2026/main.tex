...
mapper families, four scale pairs, five alternative students, and 162 recorded
evaluation runs, translation does not lift a strong student above its own
...
high for keys ($+0.81$) and partial for values ($+0.32$), consistent with a
per-head information deficit rather than a limit of mapper capacity. Under zero
re-prefill, we find no evidence that the teacher's answer-relevant advantage is
recoverable from its cache by the tested translators, and we release the
protocol as a checklist for future claims.
\end{abstract}
...
a correction loss. Under the strict regime, we measure gold-probability changes
of $-0.25$ to $-0.30$ for the strong student; the gap is a property of the
regime, not a contradiction of those results.
...
the cross-architecture and token-aligned students, the long-context task, the
...
head handling, and position accounting are therefore correct, and every
subsequent gap is attributable to the translation itself. The control holds
...
above $-\epsilon$), \emph{deg.}\ (CI entirely below zero, i.e.\ significant
degradation), \emph{n.e.}\ (CI includes zero, so replacement is not
established); ranges are across independent trainings, whose number is given in
the row label. Calibration budgets above the pool size ($200$ examples once the
fixed evaluation set is held out) saturate and reproduce the full-pool result.
...
Joint MLP, c=30 (4 trainings), 1.7B$^\ddagger$ & 0.200 / 0.224 / 29.6 & $-0.30$ to $-0.26$ & 30 & deg. \\
...
Affine per-head, c=30, long-context$^\ddagger$ & 0.233 / 0.232 / 6554.1 & $-0.285$ [$-0.534, -0.025$] & 30 & deg. \\
\bottomrule
\end{tabular}
\end{table}
...
non-inferiority would take on the order of $500$ evaluation samples, roughly
eight times the $63$ we score, whereas the 8B$\to$0.6B point estimate
($-0.023$) already sits at the margin and cannot clear it at any sample size.

\emph{Evidence tier: the long-context task, the cross-architecture students, and
...
excluded). The identity control and the student prefill sit at low perplexity
and high accuracy, while translated caches, probe mixtures, and native content
...
linear and nonlinear families and a $37$M-parameter joint MLP, but it does not
...
long-context task loader is \texttt{needle\_\allowbreak mcqa} in
