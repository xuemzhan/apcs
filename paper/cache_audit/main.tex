...
\author{%
  Anonymous\\
  \normalsize Draft, compiled \today\\
  \normalsize Code and all run artifacts: \texttt{apcs} repository (21 recorded GPU runs)
}
\date{}
...
spans $-0.14$ to $-0.30$ against a self-kv control at $+0.000$. The weak
...
communication~\citep{lsc2026}, selective KV
sharing~\citep{kvcomm2025}, and a closed-form ridge mapper that retains
73--98\% of a receiver's standalone-prefill accuracy on four of six
matched-KV pairs~\citep{heo2026crosskv}.
...
bounds every translator that could be trained. Figure~\ref{fig:framework}
...
change over the student's own prefill spans $-0.138$ to $-0.302$, and the best
configuration reaches $0.365$ against a student baseline of $0.503$; the weak
...
\begin{enumerate}
\item \textbf{An audit framework.} Identity controls, disjoint
...
without training anything (Section~\ref{sec:method}).
\item \textbf{A bounded design space.} Five mapper families, a calibration
...
(Section~\ref{sec:method} and Section~\ref{sec:results}).
\item \textbf{A verdict with consequences.} Mechanics passes, replacement
...
(Section~\ref{sec:results} and Section~\ref{sec:implications}).
\end{enumerate}
...
generation~\citep{mot2026}. A parallel recent system studies cross-model KV
cache transfer inside one family under the strict zero-re-prefill regime we
audit: it fits a closed-form, per-head ridge mapper that (i) concatenates
the top-$k$ most predictive source layers for each target layer, (ii)
strips RoPE from the keys before mapping so the fit is position-free and
reusable across context lengths, and (iii) calibrates on 500 FineWeb-Edu
sequences of 1{,}024 tokens each; across six matched-KV pairs in three
families it retains 73--98\% of the receiver's standalone-prefill accuracy
on four pairs while two degrade sharply, and an MLP recovers up to $+37$~pp
HellaSwag retention on the failures~\citep{heo2026crosskv}. KVComm shares attention-ranked KV
...
translations~\citep{hcache2025}. Latent Cache Flow translates sharer KV to
a receiver through learned adapters and targets compact, bandwidth-efficient
adapters~\citep{latentcacheflow2026}. Semantic Cache Distillation replaces
raw KV with compact semantic codes: most layers are reconstructed by a
low-rank cross-model projection, while a few key layers regenerate the
receiver's KV from the receiver's own weights, framing state transfer as
rate--distortion~\citep{semcache2026}. DroidSpeak shares KV caches between
fine-tuned variants of one architecture by selectively recomputing a few
layers and reusing the rest, so it does not address arbitrary large-to-small
state synthesis~\citep{droidspeak2024}. A recent survey, Beyond Tokens,
unifies these latent-communication lines and names cross-architecture
alignment as open~\citep{beyondtokens2026}. Two properties of this line matter for our
...
The closed-form ridge mapper above is the exception on the prefill axis
(its receiver skips prefill), but it still reports retention against a
standalone prefill without isolating the three hypotheses. Second, the
line's other strongest variants do not operate under strict zero re-prefill:
...
Heo et al.~\citep{heo2026crosskv} & matched-KV full cache & per-head ridge,
de-RoPE & no & retention vs standalone; no identity control / probe \\
...
determine caches \\
Latent Cache Flow~\citep{latentcacheflow2026} & KV via adapters & learned
adapter & no & compact adapter preserves receiver behavior \\
SCD~\citep{semcache2026} & semantic codes (Reuse+Patch) & low-rank +
receiver weights & partial (Patch) & rate-distortion suffices as proxy \\
DroidSpeak~\citep{droidspeak2024} & KV (same architecture) & selective
recompute + reuse & yes (selective) & fine-tuned variants share cache
geometry \\
\midrule
This paper & full cache (zero re-prefill) & five families incl.\ RAT & no &
...
\textbf{H2}: some $g_\theta$ reaches replacement level (gate: $\chg > 0$ with
CI excluding zero, across the mapper ladder at calibration budgets up to 200
examples). \textbf{H3}: the teacher's cache contains student-readable
...
We audit five families under one interface. \textbf{Ridge (per-head)}: a
...
improvement over $\alpha{=}0$ is a lower bound on what a perfect translator
...
bootstrap CIs. No family reaches zero; more calibration data moves affine
mapping from $-0.138$ to $-0.239$; $\lambda$ is inert over two orders of
magnitude; the architecture-anchored RAT sits inside the same band.}
\label{fig:landscape}
\end{figure}
...
single gain: moving from per-head ridge ($-0.296$) to per-head affine
($-0.138$) halves the deficit, which confirms that the teacher/student value
distributions differ by location and scale before they differ by content
(the measured teacher/student norm ratios are $1.58\times$ for keys and
$7.44\times$ for values). Second, the ladder is non-monotone in data: at
200 calibration examples the affine map degrades to $-0.239$ from $-0.138$ at
30, and $\lambda$ over $[10^{-3}, 10^{-1}]$ changes nothing. The per-head
...
solution. Third, structure changes little: per-layer merging ($-0.275$),
task-aware fine-tuning ($-0.216$), and RAT ($-0.268$ at $c{=}30$,
$-0.221$ at $c{=}200$) sit inside the same band. The strong student never
...
harmful ($0.2653$, $0.2242$). The reading is architectural. Answer-relevant
...
\textbf{For claims.} Any cache-translation evaluation should ship an identity
...
\textbf{For deployment.} Two regimes survive the audit. For weak students
...
$0.413$ against the student's $0.508$ full-text baseline, so compression
loses less than translation loses on the strong student.

\textbf{For the direction.} The survivable versions of the idea relocate
