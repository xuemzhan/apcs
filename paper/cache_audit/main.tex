...
\author{%
  ...
  \normalsize Code and all run artifacts: \texttt{apcs} repository (all runs recorded in the repository)
}
\date{}
...
$7.44\times$ for values). Second, the calibration budget is inert. Under a controlled comparison
that fixes the evaluation set ($n{=}100$) and varies only the calibration
budget, affine mapping yields CHG $-0.249$ at $c{=}30$ and $-0.244$ at
$c{=}200$: a difference of $0.005$. The mapper has converged; more
...
($-0.275$), task-aware fine-tuning ($-0.216$), RAT ($-0.221$ to $-0.268$),
and a per-head MLP mapper with two GELU layers ($-0.303$ at $c{=}30$,
$-0.236$ at $c{=}200$) all sit inside the same band. Nonlinearity does not
