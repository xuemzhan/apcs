...
# 49. 2026-08-29 Task-Aware Mapper 实验发现

## 实验结果

### 4B→1.7B Task-Aware Mapper（20 samples, 50 calib samples）

| Method | Score | CHG |
|--------|-------|-----|
| Teacher | 0.781 | - |
| Student | 0.691 | - |
| Ridge KV Both | 0.794 | +0.103 |
| Task-Aware KV Both | 0.794 | +0.103 |

**关键发现：Task-Aware Mapper 没有比 Ridge Mapper 产生任何改进。**

### 训练过程分析
- K mapper: loss 从 8.75 降至 3.26（30步），但评估分数无变化
- V mapper: loss 从 14.83 略降至 15.04（几乎不收敛），评估分数无变化
- 每个 batch 仅 4/20 样本成功完成 forward pass（n_ok=4）

### 根因分析
1. **Mapper 的瓶颈是 KV 兼容性（维度/表示），不是优化目标**
   - Ridge mapper 已经最大化了 KV 重建质量（Gram aggregate per-layer Ridge）
   - 梯度优化在 next-token prediction loss 上无法超越这个最优解
   
2. **Student 自身能力限制**
   - 1.7B student 已经同意 teacher 60% 的时间
   - Ridge mapper 改变了 85% 的 decisions，但导致 agreement 降至 35%
   - Task-aware 优化无法克服这个结构性矛盾

3. **优化信号弱**
   - 单 token cross-entropy loss 作为训练信号太弱
   - 每 batch 仅 4 个有效样本，梯度估计噪声大
   - 30 步优化不足以收敛到有意义的改进

## 结论
- 当前 Ridge mapper 已接近跨模型 KV handoff 的性能上限
- 需要根本性的方法突破（如 Causal Advantage Subspace）才能进一步提升
- Task-aware mapper 只有在 student 能力与 teacher 差距足够大时才可能有效（如 4B→0.6B）
