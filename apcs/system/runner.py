...
def _cache_bytes(L: int, S: int, H: int, D: int, dtype_bytes: int = 2) -> int:
    """§38 cache_bytes：单层 KV cache 占用的字节数（bfloat16=2 字节）。

    公式：bytes = L × S × H × D × dtype_bytes × 2 (K + V)
    """
...
def run_system_cost(cfg: dict[str, Any], run_dir) -> dict[str, Any]:
    ...
    # §38 VRAM / RAM（离线估值；真实实验读 torch.cuda + psutil）
